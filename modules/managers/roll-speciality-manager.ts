...
export class RollSpecialityManager {
    ...
    static async rollArrowVolley(token) {
        ...
        return new Promise(async resolve => {
            ...
            let successCount = 0;
            let rollsResult: ('epicFail' | 'fail' | 'success')[] = [];
            ...
            let content = `${message} ${await roll.render()}`
            ...
            if (successCount) {
                ...
                await ChatMessage.create({...damageMessageData, content: `${damageMessage} ${await rollDamage.render()}`});
